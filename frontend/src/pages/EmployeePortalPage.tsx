...
export function EmployeePortalPage({ lang }: { lang: Lang }) {
  ...
  return (
    <div className="col" style={{ gap: 12 }}>
      ...
      <Routes>
        <Route path="/" element={<div className="card">Welcome.</div>} />
        <Route path="faqs" element={<FaqsPage lang={lang} token={token!} />} />
        <Route path="documents" element={<DocumentsPage lang={lang} token={token!} />} />
        <Route path="integrations" element={<IntegrationsPage lang={lang} token={token!} />} />
        <Route path="website" element={<WebsiteCrawlPage lang={lang} token={token!} />} />
        <Route path="analytics" element={<AnalyticsPage lang={lang} token={token!} />} />
        <Route path="test" element={<ChatTestPage lang={lang} token={token!} />} />
      </Routes>
    </div>
  );
}
