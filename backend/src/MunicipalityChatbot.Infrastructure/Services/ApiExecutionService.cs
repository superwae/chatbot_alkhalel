...
) : IApiExecutionService
{
    ...
    public async Task<ApiExecutionResult> ExecuteAsync(ApiDefinition api, PlannerApiCall apiCall, CancellationToken ct, string? userToken = null)
    {
        ...
        if (!Uri.TryCreate(api.BaseUrl, UriKind.Absolute, out var baseUri))
            return new ApiExecutionResult(false, null, "", "Invalid API baseUrl.");

        if (!string.Equals(baseUri.Host, api.AllowlistedDomain, StringComparison.OrdinalIgnoreCase))
            return new ApiExecutionResult(false, null, "", "API baseUrl host is not allowlisted.");

        // Guardrails: only allow calling the saved apiId
        if (!Guid.TryParse(apiCall.ApiId, out var apiId) || apiId != api.ApiId)
            return new ApiExecutionResult(false, null, "", "API call is not allowed.");
        ...
        HttpRequestMessage CreateRequest()
        {
            ...
            // Auth based on env var references or user-provided token
            ApplyAuth(req, api, userToken);

            // Body
            if (method != HttpMethod.Get && method != HttpMethod.Head)
            {
                req.Content = ApiHttpClient.JsonContent(requestBody);
            }
        ...
        while (attempts <= options.MaxRetries)
        {
            try
            {
                ...
                return new ApiExecutionResult(status is >= 200 and < 300, status, body, status is >= 200 and < 300 ? null : $"API returned status {status}");
            }
        ...
        return new ApiExecutionResult(false, null, "", lastEx?.Message ?? "API execution failed.");
    }

    private static void ApplyHeaders(HttpRequestMessage req, string headersTemplateJson)
    {
        try
        {
